	...
	for(var i = 0; i<movieArray.length ; i++) {
		...
		for(var k =0; k<arr.length; k++){
			if(movieArray[i] == movie_shows[j].movie.m_name  && parseInt(movie_shows[j].ms_price) == parseInt(arr[k])) {
				movieRow.append($("<td>").html('<a href="/SilverScreenMavenProject/selectSeats/'+movie_shows[j].ms_id+'">' + (new Date(movie_shows[j].ms_timestamp).getHours()<10?'0':'') +new Date(movie_shows[j].ms_timestamp).getHours() + ":" +(new Date(movie_shows[j].ms_timestamp).getMinutes()<10?'0':'')+new Date(movie_shows[j].ms_timestamp).getMinutes()+"</a>"));}
		}
		}}
	...
	for(var i = 0; i<movieArray.length ; i++) {
		...
		for(var j = 0; j<movieShowForDay.length ; j++) {
			if(movieArray[i] == movieShowForDay[j].movie.m_name ) {
				movieRow.append($("<td>").html('<a href="/SilverScreenMavenProject/selectSeats/'+movieShowForDay[j].ms_id+'">' + (new Date(movieShowForDay[j].ms_timestamp).getHours()<10?'0':'') +new Date(movieShowForDay[j].ms_timestamp).getHours() + ":" +(new Date(movieShowForDay[j].ms_timestamp).getMinutes()<10?'0':'')+new Date(movieShowForDay[j].ms_timestamp).getMinutes()+"</a>"));}
		}
...
<body>
	<div class="container-fluid">
		...
		<form class="col-sm-9 col-sm-offset-2 col-xs-9 col-xs-offset-2"
			role="search" name="searchForm" style="width: 60%"
			action="/SilverScreenMavenProject/searchBox">
			<div class="input-group" style="display: block;">
				<a href="/SilverScreenMavenProject/searchBox"><input
							...
							<form id="signup-form" name="signup_form" method="Get">
								...
								<div class="form-group col-md-offset-2 col-md-8">
									...
									<input type="password" class="form-control " name="Spwd"
										id="Spwd" placeholder="Enter your password" autocomplete="off"
